...
- 集合默认请求头：集合配置默认 Headers → 集合内接口发送时自动继承合并
...
在集合的「新建/编辑集合」弹窗中可配置默认请求头，集合内接口发送时按以下规则生效：

1. 发送时先按当前环境展开 `{{变量}}`，再将默认头与接口头**按名称忽略大小写合并**：
   - 接口同名且值非空 → 覆盖默认头
   - 接口同名且值明确留空 → 剔除该默认头
   - 禁用项（默认头或接口头）不发送
2. 默认头名称非法（非 HTTP token 字符）或同名重复（忽略大小写）时**拒绝保存**，原配置不变
3. 变量缺失或合并结果无效（名称非法、值含换行等）时**阻止整次发送**并列出全部原因
4. 默认头变更只影响后续发送；请求历史按实际发出的请求回填，刷新页面后配置与结果一致
...
- 集合: 示例 API（含默认请求头 `Accept: application/json`）
- 接口:
